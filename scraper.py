...
async def get_last_n_items(city: str, n: int = 13) -> tuple[str, list[dict]]:
    ...
    for item in items[0:n]:
        try:
            ...
            link = f"https://olx.pl/{item_url}" if not item_url.startswith("https://www.otodom.pl") else item_url
            links.append(link)
...
def parse_otodom(response: requests.Response) -> dict:
    ...
    try:
        ...
        publication_time = item.find("p", {"class": "e2md81j2 css-htq2ld"}).text.split(" ")[-1]
        features = [
